 ...
 * @details 该类绑定一个 Stream（如 Serial）对象，在调用 Update() 时会读取流中所有可用数据，
 *          通过 RobustFrame::Parser 同步帧边界，识别有效载荷类型（0x01），
 *          并将数据反序列化为 gamepad::input::State，最终更新内部 Tracker。
 */
/**
 * @~English
 * @brief Decoder class responsible for parsing gamepad input frames from a byte stream and updating the state tracker.
 * @details Binds to a Stream (e.g., Serial) object. When Update() is called, it reads all available data,
 *          uses RobustFrame::Parser to synchronize frame boundaries, identifies the payload type (0x01),
 *          deserializes data into gamepad::input::State, and finally updates the internal Tracker.
 */
class Decoder {
 public:
  /**
